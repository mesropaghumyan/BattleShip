...
- **`Ship.Cells` garde la référence de la liste fournie par l'appelant, sans copie défensive** — trouvé pendant `bmad-walkthrough` (Detail Pass), medium (rattaché à AD-5 : le domaine doit rester sûr même appelé directement). Corrigé : `Cells = cells.ToList()`. 19/19 tests toujours au vert.
